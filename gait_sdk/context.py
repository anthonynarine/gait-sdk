# Filename: gait_sdk/context.py
"""
gait_sdk.context — SecurityContext (identity composition)
====================================================================
...
inferring either from the other:
    ...
    HUMAN identity      -> ClaimsUser / user claims (gait_sdk.django.
                            authentication, gait_sdk.fastapi.dependencies)
    APPLICATION identity -> ApplicationPrincipal (gait_sdk.application)
...
`gait_sdk.application`), so `SecurityContext.application` is
validated by a real `isinstance` check.

`ClaimsUser`, however, currently lives in
`gait_sdk.django.authentication`, which unconditionally imports
...
necessary, and it risks the existing stable `gait_sdk.authentication.
ClaimsUser` / `gait_sdk.django.authentication.ClaimsUser` import
paths.
...
what `gait_sdk.fastapi.dependencies.verify_token()` already
...
# The same field set gait_sdk.django.authentication.BaseUserClaims/
    ...
    def __post_init__(self) -> None:
        ...
        if self.application is not None and not isinstance(self.application, ApplicationPrincipal):
            raise TypeError(
                ...
                "(from gait_sdk.application.verify_application()) or None."
            )
