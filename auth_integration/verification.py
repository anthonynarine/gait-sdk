# Filename: auth_integration/verification.py
"""
auth_integration.verification -- Token verification core (0.4.0)
=================================================================
...
auth_integration.session.check_session_live(), which asks Gait live.
"""
...
logger = logging.getLogger("auth_integration.verification")
...
class IntrospectionVerifier:
    ...
    async def averify(self, token: str) -> VerifiedIdentity:
        if self._validate is not None:
            ...
            from auth_integration import client
    ...
    if _verifier is None:
        with _verifier_lock:
            if _verifier is None:
                ...
                logger.info("auth_integration token verifier: %s", _verifier.name)
    return _verifier
