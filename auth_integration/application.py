# Filename: auth_integration/application.py
"""
auth_integration.application — Gait Application (machine) identity
====================================================================

SDK2: adds Gait's machine/software identity to auth_integration, kept
strictly independent of — and never merged with — human identity.

Framework-agnostic, deliberately mirroring `auth_integration.client`'s own
structure and style:
...
-----------------------------------------------------------------------
- `ClaimsUser` / user claims (`auth_integration.django.authentication`,
  `auth_integration.fastapi.dependencies`) = HUMAN identity, derived from
...
from auth_integration.settings import GAIT_APPLICATION_CREDENTIAL, GAIT_AUTH_URL, GAIT_TIMEOUT

# -----------------------------------------------------------------------------
# ⚙️ Logger (never logs the raw credential — see verify_application)
# -----------------------------------------------------------------------------
logger = logging.getLogger("auth_integration.application")
...
def _build_application_principal(raw: Any) -> ApplicationPrincipal:
    ...
    auth_integration.client.validate_token's own malformed-JSON handling).
    """
