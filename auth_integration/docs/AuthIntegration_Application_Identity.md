...
Read the same way as `GAIT_AUTH_URL`/`GAIT_TIMEOUT` (`auth_integration.settings`
...
- **Never logged.** `auth_integration.settings._get_setting()` only ever
  ...
  `auth_integration.application.verify_application()` never interpolates
...
`verify_application()` is `async`, matching `auth_integration.client.validate_token`'s
