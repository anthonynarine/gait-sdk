...
`ClaimsUser` currently lives in `auth_integration.django.authentication`,
...
`isinstance` check would force `auth_integration.context` — meant to be
...
risks the existing stable `auth_integration.authentication.ClaimsUser` /
`auth_integration.django.authentication.ClaimsUser` import paths. Instead,
`auth_integration/context.py` imports `ClaimsUser` only under
