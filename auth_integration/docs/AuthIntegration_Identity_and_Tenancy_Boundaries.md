...
These are three different questions, and this package (`auth_integration`, the Gait
SDK) must keep answering only the first one:

| Axis | Question | Owner today |
|---|---|---|
| **User identity** | Who is making this request? | `auth_integration` (this package) — `ClaimsUser` / `UserClaims`, validated against Gait `/whoami/` |
...
1. **`auth_integration`'s authentication contract stays tenant-agnostic.** `ClaimsUser` /
   `UserClaims` (`auth_integration/django/authentication.py`) continue to carry only
