...
`auth_integration.verification` and `auth_integration.session`.
...
| `auth_integration` | Verification and normalization. The result is a `VerifiedIdentity`. |
...
Entry points:
- `auth_integration.session.check_session_live(token, expected_subject=...)` and its async twin `acheck_session_live`.
- Django: `auth_integration.django.authentication.require_live_session(request)`.
- FastAPI: `Depends(auth_integration.fastapi.dependencies.require_live_session)`.
